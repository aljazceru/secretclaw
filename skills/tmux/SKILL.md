...
metadata:
  { "secretclaw": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (SecretClaw)
...
```bash
SOCKET_DIR="${SECRETCLAW_TMUX_SOCKET_DIR:-${OPENCLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/secretclaw-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/secretclaw.sock"
SESSION=secretclaw-python
...
- Use `SECRETCLAW_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$SECRETCLAW_TMUX_SOCKET_DIR/secretclaw.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SECRETCLAW_TMUX_SOCKET_DIR`).
