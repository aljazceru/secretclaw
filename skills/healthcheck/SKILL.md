---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SecretClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SecretClaw cron scheduling for periodic checks, or version status checks on a machine running SecretClaw (laptop, workstation, Pi, VPS).
---

# SecretClaw Host Hardening

## Overview

Assess and harden the host running SecretClaw, then align it to a user-defined risk tolerance without breaking access. Use SecretClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SecretClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. SecretClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SecretClaw can access sensitive data.
...
### 2) Run SecretClaw security audits (read-only)

As part of the default read-only checks, run `secretclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `secretclaw security audit` (faster, non-probing)
2. `secretclaw security audit --json` (structured output)

Offer to apply SecretClaw safe defaults (numbered):

1. `secretclaw security audit --fix`

Be explicit that `--fix` only tightens SecretClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SecretClaw version/update status (read-only)

As part of the default read-only checks, run `secretclaw update status`.
...
- Credential hygiene notes (location of SecretClaw creds, prefer disk encryption)
...
- SecretClaw security audit (re-run)
...
After SecretClaw install or first hardening pass, run at least one baseline audit and version check:

- `secretclaw security audit`
- `secretclaw security audit --deep`
- `secretclaw update status`

Ongoing monitoring is recommended. Use the SecretClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `secretclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `secretclaw update status`
...
Before creating, `secretclaw cron list` and match on exact `name`. If found, `secretclaw cron edit <id> ...`.
If not found, `secretclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `secretclaw update status` (preferred for source checkouts and channels)
2. `npm view secretclaw version` (published npm version)

## SecretClaw command accuracy

Use only supported commands and flags:

- `secretclaw security audit [--deep] [--fix] [--json]`
- `secretclaw status` / `secretclaw status --deep`
- `secretclaw health --json`
- `secretclaw update status`
- `secretclaw cron add|list|runs|run`

Do not invent CLI flags or imply SecretClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SecretClaw compaction:
