...
SecretClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
secretclaw channels login --channel zalouser
...
```bash
secretclaw message send --channel zalouser --target <threadId> --message "Hello from SecretClaw!"
```
...
```bash
secretclaw channels login --channel zalouser              # Login via QR
secretclaw channels login --channel zalouser --account work
secretclaw channels status --probe
secretclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
secretclaw directory self --channel zalouser
secretclaw directory peers list --channel zalouser --query "name"
secretclaw directory groups list --channel zalouser --query "work"
secretclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
secretclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
secretclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `secretclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
secretclaw channels login --channel zalouser --account work
secretclaw message send --channel zalouser --account work --target <id> --message "Hello"
