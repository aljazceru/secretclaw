...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["secretclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["secretclaw", "pollId"]) ??
    readNestedString(value, ["secretclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "secretclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "secretclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          secretclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "secretclaw poll vote",
          displayText: "Vote recorded",
          value: { secretclawPollId: pollId, pollId },
        },
      },
    ],
  };
