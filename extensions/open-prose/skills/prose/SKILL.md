...
metadata: { "secretclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SecretClaw `sessions_spawn`
- **File I/O** == SecretClaw `read`/`write`
- **Remote fetch** == SecretClaw `web_fetch` (or `exec` with curl when POST is required)
