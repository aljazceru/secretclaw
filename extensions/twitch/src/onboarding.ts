...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: SECRETCLAW_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: SecretClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var SECRETCLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.SECRETCLAW_TWITCH_ACCESS_TOKEN?.trim();
