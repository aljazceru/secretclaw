/**
 * Twitch channel plugin for SecretClaw.
 *
...
import { buildChannelConfigSchema } from "secretclaw/plugin-sdk";
import type {
 ...
 * for SecretClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SecretClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SecretClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SecretClawConfig): boolean => {
