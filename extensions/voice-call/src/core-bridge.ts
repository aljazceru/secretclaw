...
function resolveSecretClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SECRETCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SECRETCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSecretClawRoot(), "dist", "extensionAPI.js");
