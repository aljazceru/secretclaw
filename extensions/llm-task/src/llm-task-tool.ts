...
// NOTE: This extension is intended to be bundled with SecretClaw.
// When running from source (tests/dev), SecretClaw internals live under src/.
...
export function createLlmTaskTool(api: SecretClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via secretclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-llm-task-"));
