...
`secretclaw.invoke --each`) without adding new SecretClaw code per workflow.
...
This extension depends on SecretClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SecretClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.secretclaw/extensions` as a standalone plugin directory.
