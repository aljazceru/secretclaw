import type { PluginRuntime } from "secretclaw/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SECRETCLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-nostr-"));
  process.env.SECRETCLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.SECRETCLAW_STATE_DIR?.trim() || env.SECRETCLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".secretclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SECRETCLAW_STATE_DIR = previous;
    }
