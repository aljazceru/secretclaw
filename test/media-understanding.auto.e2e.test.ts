...
import type { SecretClawConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("secretclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("secretclaw-bin-sherpa-");
      const modelDir = await makeTempDir("secretclaw-sherpa-model-");
      ...
      const cfg: SecretClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("secretclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("secretclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("secretclaw-bin-gemini-");
      ...
      const cfg: SecretClawConfig = { tools: { media: { image: {} } } };
