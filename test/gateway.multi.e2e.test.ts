...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `secretclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".secretclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SECRETCLAW_GATEWAY_TOKEN: inst.gatewayToken,
        SECRETCLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SECRETCLAW_GATEWAY_PORT: String(gwA.port),
          SECRETCLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          SECRETCLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SECRETCLAW_GATEWAY_PORT: String(gwB.port),
          SECRETCLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          SECRETCLAW_GATEWAY_PASSWORD: "",
        }),
