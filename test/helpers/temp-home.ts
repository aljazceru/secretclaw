...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SECRETCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SECRETCLAW_STATE_DIR = path.join(base, ".secretclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "secretclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".secretclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
