...
const originalConnect = SecretClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("secretclaw-app") as SecretClawApp;
...
beforeEach(() => {
  SecretClawApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SECRETCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SecretClawApp.prototype.connect = originalConnect;
  window.__SECRETCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/secretclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/secretclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/secretclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SECRETCLAW_CONTROL_UI_BASE_PATH__ = "/secretclaw";
    const app = mountApp("/secretclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/secretclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/secretclaw/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "secretclaw.control.settings.v1",
