...
declare global {
  interface Window {
    __SECRETCLAW_ASSISTANT_NAME__?: string;
    __SECRETCLAW_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__SECRETCLAW_ASSISTANT_NAME__,
    avatar: window.__SECRETCLAW_ASSISTANT_AVATAR__,
  });
}
