...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `secretclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SECRETCLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.SECRETCLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[secretclaw] Gateway daemon failed:",
