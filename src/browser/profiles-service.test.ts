...
vi.mock("./chrome.js", () => ({
  resolveSecretClawUserDataDir: vi.fn(() => "/tmp/secretclaw-test/secretclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSecretClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "secretclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSecretClawUserDataDir).mockReturnValue(userDataDir);
