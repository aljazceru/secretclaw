import type { BrowserConfig, BrowserProfileConfig, SecretClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SECRETCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_SECRETCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "secretclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SECRETCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SECRETCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SecretClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for secretclaw/secretclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SecretClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SECRETCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "secretclaw";
