...
const RELAY_AUTH_HEADER = "x-secretclaw-relay-token";
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/SecretClaw-Extension-Relay",
          revision: "0",
          userAgent: "SecretClaw-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "SecretClaw/extension-relay",
