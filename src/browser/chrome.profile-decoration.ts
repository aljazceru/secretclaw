...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".secretclaw-profile-decorated");
}
 ...
 */
export function decorateSecretClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SECRETCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SECRETCLAW_BROWSER_COLOR).toUpperCase();
