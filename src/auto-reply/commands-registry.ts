...
import type { SecretClawConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: SecretClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SecretClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SecretClawConfig),
...
export function getCommandDetection(_cfg?: SecretClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SecretClawConfig) {
