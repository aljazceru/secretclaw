...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(async (home) => await fn(home), { prefix: "secretclaw-triggers-" });
}
...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withTempHome(async (home) => {
      const inboundDir = join(home, ".secretclaw", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        cfg: {
          agents: {
            defaults: {
              ...
              workspace: join(home, "secretclaw"),
              sandbox: {
                mode: "non-main",
        ...
        workspaceDir: join(home, "secretclaw"),
      });
