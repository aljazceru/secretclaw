...
type AgentConfig = Partial<NonNullable<NonNullable<SecretClawConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.secretclaw/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 SecretClaw ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SecretClawConfig): string {
  const lines = ["ℹ️ Help", ""];
