import type { AgentElevatedAllowFromConfig, SecretClawConfig } from "../../config/config.js";
...
export function formatElevatedUnavailableMessage(params: {
  ...
  if (params.sessionKey) {
    lines.push(
      `See: ${formatCliCommand(`secretclaw sandbox explain --session ${params.sessionKey}`)}`,
    );
  }
  return lines.join("\n");
}
