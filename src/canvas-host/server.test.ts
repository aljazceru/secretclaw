...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("secretclawCanvasA2UIAction");
    expect(out).toContain("secretclawSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("secretclawSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.secretclawA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__secretclaw__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("secretclaw-a2ui-host");
      expect(html).toContain("secretclawCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__secretclaw__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("secretclawA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
