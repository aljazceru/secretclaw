...
const execHostEnforced = process.env.SECRETCLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.SECRETCLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSecretClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
