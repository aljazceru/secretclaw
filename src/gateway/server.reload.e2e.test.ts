...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SECRETCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.SECRETCLAW_SKIP_GMAIL_WATCHER;
    process.env.SECRETCLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SECRETCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SECRETCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SECRETCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
