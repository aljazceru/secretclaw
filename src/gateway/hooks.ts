...
export function resolveHooksConfig(cfg: SecretClawConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-secretclaw-token"] === "string"
      ? req.headers["x-secretclaw-token"].trim()
      : "";
  if (headerToken) {
