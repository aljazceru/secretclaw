...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/secretclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SecretClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SECRETCLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "secretclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SECRETCLAW_TAILNET_DNS?.trim();
