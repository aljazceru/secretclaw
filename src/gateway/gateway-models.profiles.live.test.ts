...
import type { SecretClawConfig, ModelProviderConfig } from "../config/types.js";
...
import { ensureSecretClawModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SECRETCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SECRETCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SECRETCLAW_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: SecretClawConfig;
  agentDir: string;
}): SecretClawConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<SecretClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.SECRETCLAW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SECRETCLAW_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveSecretClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-live-state-"));
  ...
  process.env.SECRETCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.secretclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSecretClawAgentDir();
  const sanitizedCfg: SecretClawConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "SecretClaw live tool probe (local, safe): " +
    ...
    process.env.SECRETCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SECRETCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureSecretClawModelsJson(cfg);
      ...
      const rawModels = process.env.SECRETCLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.secretclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
