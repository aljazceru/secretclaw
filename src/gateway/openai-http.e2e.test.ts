    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "secretclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-secretclaw-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-secretclaw-agent-id": "beta",
            "x-secretclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
