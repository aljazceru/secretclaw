...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-gw-mock-home-"));
      ...
      process.env.SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.SECRETCLAW_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "secretclaw");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.secretclaw-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".secretclaw");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "secretclaw.json");
      ...
      process.env.SECRETCLAW_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.SECRETCLAW_CONFIG_PATH = prev.configPath;
        process.env.SECRETCLAW_GATEWAY_TOKEN = prev.token;
        process.env.SECRETCLAW_SKIP_CHANNELS = prev.skipChannels;
        process.env.SECRETCLAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.SECRETCLAW_SKIP_CRON = prev.skipCron;
        process.env.SECRETCLAW_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.SECRETCLAW_STATE_DIR = prev.stateDir;
      process.env.SECRETCLAW_CONFIG_PATH = prev.configPath;
      process.env.SECRETCLAW_GATEWAY_TOKEN = prev.token;
      process.env.SECRETCLAW_SKIP_CHANNELS = prev.skipChannels;
      process.env.SECRETCLAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
