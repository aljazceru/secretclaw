...
import { createSecretClawTools } from "../agents/secretclaw-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.SECRETCLAW_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.SECRETCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.SECRETCLAW_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SECRETCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createSecretClawTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
