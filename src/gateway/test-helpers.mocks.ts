...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `secretclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SECRETCLAW_CONFIG_PATH = path.join(root, "secretclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "secretclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "secretclaw-gateway-test"),
        ...fileDefaults,
