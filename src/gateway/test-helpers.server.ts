...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SECRETCLAW_STATE_DIR;
  previousConfigPath = process.env.SECRETCLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SECRETCLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SECRETCLAW_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SECRETCLAW_STATE_DIR = path.join(tempHome, ".secretclaw");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "secretclaw-test-"))
    : path.join(tempHome, ".secretclaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.SECRETCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.SECRETCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SECRETCLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.SECRETCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.SECRETCLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.SECRETCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SECRETCLAW_GATEWAY_TOKEN = fallbackToken;
  }
