...
import { discoverSecretClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SecretClawPluginDefinition;
  register?: SecretClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SecretClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SecretClawPluginDefinition;
...
export function loadSecretClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSecretClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "secretclaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SecretClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as SecretClawPluginModule;
    } catch (err) {
