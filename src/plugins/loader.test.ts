...
import { loadSecretClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SECRETCLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `secretclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "secretclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SECRETCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSecretClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SECRETCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSecretClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSecretClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
