...
export function listAgentEntries(cfg: SecretClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SecretClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: SecretClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SecretClawConfig): AgentSummary[] {
