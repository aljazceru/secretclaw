...
beforeEach(() => {
  ...
  resolveSecretClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SECRETCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.SECRETCLAW_UPDATE_IN_PROGRESS;
  process.env.SECRETCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "secretclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SECRETCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SECRETCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.SECRETCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSecretClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSecretClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/secretclaw.json",
    createConfigIO,
...
vi.mock("../infra/secretclaw-root.js", () => ({
  resolveSecretClawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "secretclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SECRETCLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SECRETCLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/secretclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SECRETCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
