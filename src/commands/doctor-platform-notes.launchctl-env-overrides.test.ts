...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SECRETCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SECRETCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SECRETCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("SECRETCLAW_GATEWAY_PASSWORD");
  });
