...
function resolveMode(cfg: SecretClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("SecretClaw doctor");

  const root = await resolveSecretClawPackageRoot({
  ...
  let cfg: SecretClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("secretclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("secretclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("secretclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("secretclaw doctor --fix")}" to apply changes.`);
  }
