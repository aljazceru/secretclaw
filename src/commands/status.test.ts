...
beforeAll(() => {
  previousProfile = process.env.SECRETCLAW_PROFILE;
  process.env.SECRETCLAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.SECRETCLAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/secretclaw-root.js", () => ({
  resolveSecretClawPackageRoot: vi.fn().mockResolvedValue("/tmp/secretclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/secretclaw/pnpm-lock.yaml",
      markerPath: "/tmp/secretclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("SecretClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("secretclaw status --all") ||
          l.includes("secretclaw --profile isolated status --all") ||
          l.includes("secretclaw status --all") ||
          l.includes("secretclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SECRETCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
