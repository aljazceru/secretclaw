...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSecretClawAgentDir: vi.fn().mockReturnValue("/tmp/secretclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/secretclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/secretclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSecretClawAgentDir: mocks.resolveSecretClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSecretClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/secretclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/secretclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/secretclaw-agent-custom");
