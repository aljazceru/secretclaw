...
import { readConfigFileSnapshot, type SecretClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SecretClawConfig, model: string): SecretClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("secretclaw plugins install")}\`.`,
    );
  }
