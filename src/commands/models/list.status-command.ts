...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSecretClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("secretclaw models auth setup-token")}\` or \`${formatCliCommand("secretclaw configure")}\`.`
          : `Run \`${formatCliCommand("secretclaw configure")}\` or set an API key env var.`;
