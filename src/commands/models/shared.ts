...
export async function updateConfig(
  mutator: (cfg: SecretClawConfig) => SecretClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SecretClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SecretClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("secretclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
