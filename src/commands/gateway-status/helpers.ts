import type { SecretClawConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: SecretClawConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.SECRETCLAW_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.SECRETCLAW_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: SecretClawConfig) {
