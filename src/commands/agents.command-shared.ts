...
export async function requireValidConfig(runtime: RuntimeEnv): Promise<SecretClawConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("secretclaw doctor")}.`);
    runtime.exit(1);
    return null;
  }
