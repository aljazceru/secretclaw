...
import type { SecretClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SecretClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("secretclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("secretclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("secretclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("secretclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("secretclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
