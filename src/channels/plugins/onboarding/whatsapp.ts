...
function setWhatsAppDmPolicy(cfg: SecretClawConfig, dmPolicy: DmPolicy): SecretClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: SecretClawConfig, allowFrom?: string[]): SecretClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: SecretClawConfig, selfChatMode: boolean): SecretClawConfig {
...
async function detectWhatsAppLinked(cfg: SecretClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so SecretClaw can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for SecretClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("secretclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
