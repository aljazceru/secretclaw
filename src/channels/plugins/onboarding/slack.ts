...
function setSlackDmPolicy(cfg: SecretClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "SecretClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for SecretClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: SecretClawConfig, allowFrom: string[]): SecretClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
