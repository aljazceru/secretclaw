...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[secretclaw] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[secretclaw] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[secretclaw] FATAL unhandled rejection:", formatUncaughtError(reason));
      process.exit(1);
      return;
    }
    ...
    if (isConfigError(reason)) {
      console.error(
        "[secretclaw] CONFIGURATION ERROR - requires fix:",
    ...
    if (isTransientNetworkError(reason)) {
      console.warn(
        "[secretclaw] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[secretclaw] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
