...
export function triggerSecretClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SECRETCLAW_SYSTEMD_UNIT,
        process.env.SECRETCLAW_PROFILE,
      );
  ...
  const label =
    process.env.SECRETCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SECRETCLAW_PROFILE);
