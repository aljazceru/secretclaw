...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-update-check-"));
    process.env.SECRETCLAW_STATE_DIR = tempDir;
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveSecretClawPackageRoot } = await import("./secretclaw-root.js");
    ...
    vi.mocked(resolveSecretClawPackageRoot).mockResolvedValue("/opt/secretclaw");
