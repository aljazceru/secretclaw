  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "secretclaw-update-preflight-"),
      );
    ...
    const doctorStep = await runStep(
      step(
        "secretclaw doctor",
        managerScriptArgs(manager, "secretclaw", ["doctor", "--non-interactive"]),
        gitRoot,
        { SECRETCLAW_UPDATE_IN_PROGRESS: "1" },
      ),
    );
