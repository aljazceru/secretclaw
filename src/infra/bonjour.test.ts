...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.SECRETCLAW_MDNS_HOSTNAME = "test-host";
    process.env.SECRETCLAW_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("secretclaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("secretclaw (SecretClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("secretclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("secretclaw.local");
