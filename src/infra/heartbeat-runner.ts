...
function resolveActiveHoursTimezone(cfg: SecretClawConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SecretClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SecretClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: SecretClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: SecretClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SecretClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SecretClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SecretClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
