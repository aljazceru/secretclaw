...
function candidateBinDirs(opts: EnsureSecretClawPathOpts): string[] {
  ...
  // Bundled macOS app: `secretclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "secretclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/secretclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "secretclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `secretclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSecretClawCliOnPath(opts: EnsureSecretClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SECRETCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SECRETCLAW_PATH_BOOTSTRAPPED = "1";
