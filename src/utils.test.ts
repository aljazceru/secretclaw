...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "secretclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "secretclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "secretclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "secretclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.secretclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "secretclaw-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/secretclaw")).toBe(path.resolve(os.homedir(), "secretclaw"));
  });
