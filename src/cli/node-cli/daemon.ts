...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("secretclaw node install"),
    formatCliCommand("secretclaw node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("secretclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    SECRETCLAW_LOG_PREFIX: baseEnv.SECRETCLAW_LOG_PREFIX ?? "node",
