...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.secretclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SECRETCLAW_PROFILE = profile;

  const stateDir = env.SECRETCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.SECRETCLAW_STATE_DIR?.trim()) {
    env.SECRETCLAW_STATE_DIR = stateDir;
  }

  if (!env.SECRETCLAW_CONFIG_PATH?.trim()) {
    env.SECRETCLAW_CONFIG_PATH = path.join(stateDir, "secretclaw.json");
  }

  if (profile === "dev" && !env.SECRETCLAW_GATEWAY_PORT?.trim()) {
    env.SECRETCLAW_GATEWAY_PORT = "19001";
  }
}
