...
import { resolveSecretClawPackageRoot } from "../infra/secretclaw-root.js";
import {
  channelToNpmTag,
  DEFAULT_GIT_CHANNEL,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "secretclaw doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "secretclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const SECRETCLAW_REPO_URL = "https://github.com/secretclaw/secretclaw.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".secretclaw");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("secretclaw@")) {
    return trimmed.slice("secretclaw@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "secretclaw.mjs");
...
function resolveGitInstallDir(): string {
  const override = process.env.SECRETCLAW_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SECRETCLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SECRETCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set SECRETCLAW_GIT_DIR to an empty folder or an secretclaw checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SECRETCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveSecretClawPackageRoot({
  ...
  defaultRuntime.log(theme.heading("SecretClaw update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating SecretClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this SecretClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("secretclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("secretclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g secretclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g secretclaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SECRETCLAW_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SECRETCLAW_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("secretclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("secretclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("secretclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("secretclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `secretclaw update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `SECRETCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set SECRETCLAW_GIT_DIR to an empty folder or an secretclaw checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via SECRETCLAW_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SecretClaw to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["secretclaw update", "Update a source checkout (git)"],
        ["secretclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["secretclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["secretclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["secretclaw update --no-restart", "Update without restarting the service"],
        ["secretclaw update --json", "Output result as JSON"],
        ["secretclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["secretclaw update wizard", "Interactive update wizard"],
        ["secretclaw --update", "Shorthand for secretclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run secretclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["secretclaw update status", "Show channel + version status."],
          ["secretclaw update status --json", "JSON output."],
          ["secretclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
