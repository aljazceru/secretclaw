...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "secretclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      SECRETCLAW_NO_RESPAWN: "1",
      SECRETCLAW_STATE_DIR: stateDir,
      SECRETCLAW_SKIP_CHANNELS: "1",
      SECRETCLAW_SKIP_GMAIL_WATCHER: "1",
      SECRETCLAW_SKIP_CRON: "1",
      SECRETCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      SECRETCLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "secretclaw-entry-bootstrap.mjs");
