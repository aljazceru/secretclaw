...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "secretclaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".secretclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SecretClawConfig> = {},
): SecretClawConfig {
  const base: SecretClawConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "secretclaw"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".secretclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
