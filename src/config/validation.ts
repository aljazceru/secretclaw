...
import type { SecretClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SecretClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SecretClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SecretClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SecretClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SecretClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SecretClawConfig)),
    ),
  };
}
