...
import type { SecretClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareSecretClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): SecretClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SecretClawConfig;
}
...
function stampConfigVersion(cfg: SecretClawConfig): SecretClawConfig {
...
function warnIfConfigFromFuture(cfg: SecretClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSecretClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer SecretClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: SecretClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SecretClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as SecretClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SecretClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SecretClawConfig) {
    clearConfigCache();
...
// module scope. `SECRETCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SECRETCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SECRETCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): SecretClawConfig {
...
export async function writeConfigFile(cfg: SecretClawConfig): Promise<void> {
  clearConfigCache();
