...
const ETC_SECRETCLAW_DIR = path.join(ROOT_DIR, "etc", "secretclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "secretclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSecretClawPath(...parts: string[]) {
  return path.join(ETC_SECRETCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSecretClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "secretclaw.json"))).toEqual({
      shared: true,
    });
  });
});
