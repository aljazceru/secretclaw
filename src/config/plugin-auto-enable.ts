import type { SecretClawConfig } from "./config.js";
...
function isTelegramConfigured(cfg: SecretClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: SecretClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: SecretClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: SecretClawConfig): boolean {
...
function isIMessageConfigured(cfg: SecretClawConfig): boolean {
...
function isWhatsAppConfigured(cfg: SecretClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: SecretClawConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: SecretClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SecretClawConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: SecretClawConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: SecretClawConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: SecretClawConfig, pluginId: string): SecretClawConfig {
...
function enablePluginEntry(cfg: SecretClawConfig, pluginId: string): SecretClawConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
