...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SECRETCLAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SECRETCLAW_STATE_DIR = path.join(home, ".secretclaw");
    ...
    await fs.mkdir(path.join(home, ".secretclaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.secretclaw${path.sep}media`);
