...
import { __testing, createSecretClawCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createSecretClawTools } from "./secretclaw-tools.js";
...
describe("createSecretClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createSecretClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSecretClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "secretclaw-outside.txt");
