...
describe("resolveSecretClawAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-agent-"));
    process.env.SECRETCLAW_STATE_DIR = tempStateDir;
    delete process.env.SECRETCLAW_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveSecretClawAgentDir();

    expect(resolved).toBe(path.join(tempStateDir, "agents", "main", "agent"));
  });

  it("honors SECRETCLAW_AGENT_DIR overrides", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-agent-"));
    const override = path.join(tempStateDir, "agent");
    process.env.SECRETCLAW_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;
