...
import { createSecretClawCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("secretclaw-ws-", async (workspaceDir) => {
      await withTempDir("secretclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createSecretClawCodingTools({ workspaceDir });
    ...
    await withTempDir("secretclaw-ws-", async (workspaceDir) => {
      await withTempDir("secretclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello secretclaw");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("secretclaw-ws-", async (workspaceDir) => {
      await withTempDir("secretclaw-override-", async (overrideDir) => {
        const tools = createSecretClawCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("secretclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("secretclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "secretclaw-sbx-test",
        ...
        const tools = createSecretClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
