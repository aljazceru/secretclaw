...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "secretclaw.sandbox=1");
  args.push("--label", `secretclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `secretclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `secretclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("secretclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`secretclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`secretclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("secretclaw sandbox recreate --all");
}
