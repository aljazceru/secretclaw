...
/**
 * Cleanup suspended SecretClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SecretClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-cli-images-"));
  const paths: string[] = [];
