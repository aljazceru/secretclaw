...
const defaultTools = createSecretClawCodingTools();

describe("createSecretClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createSecretClawCodingTools();
    ...
    try {
      ...
      const contents = "Hello from secretclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "secretclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "secretclaw-workspace"),
    ...
    const tools = createSecretClawCodingTools({ sandbox });
