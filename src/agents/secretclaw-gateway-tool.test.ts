...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.SECRETCLAW_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secretclaw-test-"));
    process.env.SECRETCLAW_STATE_DIR = stateDir;
    process.env.SECRETCLAW_PROFILE = "isolated";

    try {
      const tool = createSecretClawTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: secretclaw --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/secretclaw" } }\n}\n';
