...
This directory contains hooks that ship with SecretClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.secretclaw/secretclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'secretclaw.*gateway' && pnpm secretclaw gateway`
3. Enable the hook: `secretclaw hooks enable my-hook`
