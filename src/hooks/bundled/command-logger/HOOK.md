...
2. **Appends to log file** - Writes a JSON line to `~/.secretclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.secretclaw/logs/commands.log ~/.secretclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/secretclaw`:
   ```
   /home/username/.secretclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
