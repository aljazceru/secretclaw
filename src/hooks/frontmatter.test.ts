...
`;
    ...
    expect(parsed.secretclaw.emoji).toBe("💾");
    expect(parsed.secretclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.secretclaw.emoji).toBe("📝");
    expect(parsed.secretclaw.events).toEqual(["command"]);
    expect(parsed.secretclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.secretclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"secretclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"secretclaw": {"events": ["test"]}}');
  });
...
describe("resolveSecretClawMetadata", () => {
  it("extracts secretclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSecretClawMetadata(frontmatter);
  ...
  it("returns undefined when secretclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        secretclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with SecretClaw" },
            { id: "npm", kind: "npm", package: "@secretclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@secretclaw/hook");
  });
    ...
    "secretclaw":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with SecretClaw" }],
      },
  }
---
...
`;
    ...
    const secretclaw = resolveSecretClawMetadata(frontmatter);
    expect(secretclaw).toBeDefined();
    expect(secretclaw?.emoji).toBe("💾");
    expect(secretclaw?.events).toEqual(["command:new"]);
    expect(secretclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(secretclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const secretclaw = resolveSecretClawMetadata(frontmatter);
    expect(secretclaw?.emoji).toBe("disk");
    expect(secretclaw?.events).toEqual(["command:new"]);
  });
});
