...
Deploy SecretClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/secretclaw` — Control UI
...
- `SECRETCLAW_STATE_DIR=/data/.secretclaw` (recommended)
- `SECRETCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `SECRETCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your SecretClaw state + workspace so you can migrate to another host without losing config or memory.
