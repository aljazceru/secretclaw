...
This document explains how SecretClaw manages sessions end-to-end:
...
SecretClaw is designed around a single **Gateway process** that owns session state.
...
SecretClaw persists sessions in two layers:
...
- Transcripts: `~/.secretclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

SecretClaw resolves these via `src/config/sessions.ts`.
...
SecretClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (SecretClaw consumes the events, but Pi decides when to compact).
...
SecretClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SecretClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, SecretClaw leaves it alone.
...
- `secretclaw status` (CLI)
- `secretclaw sessions` / `sessions --json`
...
SecretClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- SecretClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, SecretClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
SecretClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but SecretClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `secretclaw status`.
