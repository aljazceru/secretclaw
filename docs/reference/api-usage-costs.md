...
SecretClaw features that can generate provider usage or paid API calls.
...
- `secretclaw status --usage` and `secretclaw channels list` show provider **usage windows**
...
SecretClaw can pick up credentials from:
...
- `secretclaw status --usage`
- `secretclaw models status --json`
...
`secretclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
