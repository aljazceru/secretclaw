...
```bash
secretclaw plugins install @secretclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
secretclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "SecretClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `secretclaw pairing list nextcloud-talk`
  - `secretclaw pairing approve nextcloud-talk <CODE>`
