...
read_when:
  - Setting up Twitch chat integration for SecretClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SecretClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
secretclaw plugins install @secretclaw/twitch
```

Local checkout (when running from a git repo):

```bash
secretclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SECRETCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "secretclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SECRETCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
SECRETCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
