...
Tlon is a decentralized messenger built on Urbit. SecretClaw connects to your Urbit ship and can
...
```bash
secretclaw plugins install @secretclaw/tlon
```

Local checkout (when running from a git repo):

```bash
secretclaw plugins install ./extensions/tlon
```
...
Use these with `secretclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SecretClaw replies in-thread.
