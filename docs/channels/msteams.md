...
```bash
secretclaw plugins install @secretclaw/msteams
```

Local checkout (when running from a git repo):

```bash
secretclaw plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
SecretClaw will offer the local install path automatically.
...
3. Configure SecretClaw with those credentials.
...
- Talk to SecretClaw via Teams DMs, group chats, or channels.
...
- On startup, SecretClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.secretclaw/secretclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SecretClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                      |
   | ------------------ | ---------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `secretclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `secretclaw plugins install @secretclaw/msteams`
   - From a local checkout: `secretclaw plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "SecretClaw in Teams", "full": "SecretClaw in Teams" },
...
SecretClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SecretClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SecretClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SecretClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `secretclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.secretclaw/msteams-polls.json`.
...
```bash
secretclaw message send --channel msteams \
...
secretclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
secretclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
secretclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
secretclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
