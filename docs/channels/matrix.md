...
Matrix is an open, decentralized messaging protocol. SecretClaw connects as a Matrix **user**
...
```bash
secretclaw plugins install @secretclaw/matrix
```

Local checkout (when running from a git repo):

```bash
secretclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `secretclaw plugins install @secretclaw/matrix`
   - From a local checkout: `secretclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SecretClaw calls the same
     login endpoint, stores the access token in `~/.secretclaw/credentials/matrix/credentials.json`,
...
- On first connection, SecretClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SecretClaw logs a warning.
...
`~/.secretclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `secretclaw pairing list matrix`
  - `secretclaw pairing approve matrix <CODE>`
...
- On startup, SecretClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
