---
summary: "Chrome extension: let SecretClaw drive your existing Chrome tab"
read_when:
...
The SecretClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate secretclaw-managed Chrome profile.
...
SecretClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SecretClaw release (npm package) as static files. There is no separate “build” step.

After upgrading SecretClaw:

- Re-run `secretclaw browser extension install` to refresh the installed files under your SecretClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

SecretClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want SecretClaw to control.
...
- `ON`: attached; SecretClaw can drive that tab.
...
Debugging: `secretclaw sandbox explain`
...
`secretclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `secretclaw browser extension install` first to copy the extension to a stable location under your SecretClaw state directory.
...
- **This is not isolated** like the dedicated secretclaw-managed profile.
