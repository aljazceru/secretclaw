...
- **gateway host** → `secretclaw` process on the gateway machine
...
`~/.secretclaw/exec-approvals.json` directly.

CLI: `secretclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
