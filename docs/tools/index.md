---
summary: "Agent tool surface for SecretClaw (browser, canvas, nodes, message, cron) replacing legacy `secretclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `secretclaw-*` skills
title: "Tools"
---

# Tools (SecretClaw)

SecretClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `secretclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `secretclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, SecretClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`secretclaw node run`).
...
- Requires a Brave API key (recommended: `secretclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated SecretClaw-managed browser.
...
- Quick smoke: `secretclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `secretclaw gateway` restart in-place)
