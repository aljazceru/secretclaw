...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`secretclaw setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.secretclaw/secretclaw.json` 和智能体工作区。
...
```bash
secretclaw setup
secretclaw setup --workspace ~/.secretclaw/workspace
```

通过 setup 运行向导：

```bash
secretclaw setup --wizard
```
