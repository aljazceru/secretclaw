---
read_when:
  - 你想在 SecretClaw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 SecretClaw 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
secretclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
secretclaw gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/secretclaw/secretclaw/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `secretclaw models list` 和 `secretclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `secretclaw configure` 并选择 **MiniMax M2.1**，或
