...
你可以在 `~/.secretclaw/secretclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/secretclaw.log"
  }
}
```
...
```bash
secretclaw channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.secretclaw/secretclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/secretclaw/secretclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- SecretClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
SECRETCLAW_DIAGNOSTICS=telegram.http,telegram.payload
```
...
```json
{
  ...
  "diagnostics": {
    ...
    "otel": {
      ...
      "serviceName": "secretclaw-gateway",
      "traces": true,
      "metrics": true,
      "logs": true,
      "sampleRate": 0.2,
...
- 你也可以使用 `secretclaw plugins enable diagnostics-otel` 启用插件。
...
- `secretclaw.tokens`（计数器，属性：`secretclaw.token`、`secretclaw.channel`、`secretclaw.provider`、`secretclaw.model`）
- `secretclaw.cost.usd`（计数器，属性：`secretclaw.channel`、`secretclaw.provider`、`secretclaw.model`）
- `secretclaw.run.duration_ms`（直方图，属性：`secretclaw.channel`、`secretclaw.provider`、`secretclaw.model`）
- `secretclaw.context.tokens`（直方图，属性：`secretclaw.context`、`secretclaw.channel`、`secretclaw.provider`、`secretclaw.model`）

消息流：

- `secretclaw.webhook.received`（计数器，属性：`secretclaw.channel`、`secretclaw.webhook`）
- `secretclaw.webhook.error`（计数器，属性：`secretclaw.channel`、`secretclaw.webhook`）
- `secretclaw.webhook.duration_ms`（直方图，属性：`secretclaw.channel`、`secretclaw.webhook`）
- `secretclaw.message.queued`（计数器，属性：`secretclaw.channel`、`secretclaw.source`）
- `secretclaw.message.processed`（计数器，属性：`secretclaw.channel`、`secretclaw.outcome`）
- `secretclaw.message.duration_ms`（直方图，属性：`secretclaw.channel`、`secretclaw.outcome`）

队列 + 会话：

- `secretclaw.queue.lane.enqueue`（计数器，属性：`secretclaw.lane`）
- `secretclaw.queue.lane.dequeue`（计数器，属性：`secretclaw.lane`）
- `secretclaw.queue.depth`（直方图，属性：`secretclaw.lane` 或 `secretclaw.channel=heartbeat`）
- `secretclaw.queue.wait_ms`（直方图，属性：`secretclaw.lane`）
- `secretclaw.session.state`（计数器，属性：`secretclaw.state`、`secretclaw.reason`）
- `secretclaw.session.stuck`（计数器，属性：`secretclaw.state`）
- `secretclaw.session.stuck_age_ms`（直方图，属性：`secretclaw.state`）
- `secretclaw.run.attempt`（计数器，属性：`secretclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `secretclaw.model.usage`
  - `secretclaw.channel`、`secretclaw.provider`、`secretclaw.model`
  - `secretclaw.sessionKey`、`secretclaw.sessionId`
  - `secretclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `secretclaw.webhook.processed`
  - `secretclaw.channel`、`secretclaw.webhook`、`secretclaw.chatId`
- `secretclaw.webhook.error`
  - `secretclaw.channel`、`secretclaw.webhook`、`secretclaw.chatId`、`secretclaw.error`
- `secretclaw.message.processed`
  - `secretclaw.channel`、`secretclaw.outcome`、`secretclaw.chatId`、`secretclaw.messageId`、`secretclaw.sessionKey`、`secretclaw.sessionId`、`secretclaw.reason`
- `secretclaw.session.stuck`
  - `secretclaw.state`、`secretclaw.ageMs`、`secretclaw.queueDepth`、`secretclaw.sessionKey`、`secretclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `secretclaw doctor`。
