...
SecretClaw 提供两个安装器脚本（托管在 `openclaw.ai`）：
...
如果安装器完成但在新终端中找不到 `secretclaw`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g secretclaw@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在 Linux 上：必要时将 npm 前缀切换到 `~/.npm-global`，以避免全局 npm 权限错误。
- 如果是升级现有安装：运行 `secretclaw doctor --non-interactive`（尽力执行）。
- 对于 git 安装：安装/更新后运行 `secretclaw doctor --non-interactive`（尽力执行）。
...
如果你在**已有的 SecretClaw 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：

- 更新并使用此检出（`git`）
- 或迁移到全局 npm 安装（`npm`）

在非交互式上下文中（无 TTY / `--no-prompt`），你必须传入 `--install-method git|npm`（或设置 `SECRETCLAW_INSTALL_METHOD`），否则脚本将以退出码 `2` 退出。
...
此脚本将 `secretclaw` 安装到前缀目录（默认：`~/.secretclaw`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g secretclaw@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在升级和 git 安装时运行 `secretclaw doctor --non-interactive`（尽力执行）。
