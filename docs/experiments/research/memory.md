...
read_when:
  - Designing workspace memory (~/.secretclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SecretClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.secretclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.secretclaw/workspace` as canonical human-readable memory.
...
```
~/.secretclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SecretClaw**, but keep a separable core library.

### Why integrate into SecretClaw?

- SecretClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `secretclaw memory recall "…" --k 25 --since 30d`
  - `secretclaw memory reflect --since 7d`
...
Pragmatic take for `~/.secretclaw/workspace`:
