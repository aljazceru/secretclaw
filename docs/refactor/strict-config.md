...
- Plugin manifests (`secretclaw.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`secretclaw.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `secretclaw doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `secretclaw doctor --fix`.”
