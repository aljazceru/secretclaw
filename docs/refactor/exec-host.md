...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.secretclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.secretclaw/node.json` (node id + pairing token).
...
Path: `~/.secretclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.secretclaw/exec-approvals.sock",
...
- Unix socket at `~/.secretclaw/exec-approvals.sock` (0600).
