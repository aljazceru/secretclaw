...
- Publish as `secretclaw/plugin-sdk` (or export from core under `secretclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SecretClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SecretClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SecretClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SecretClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SecretClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SecretClawConfig): string;
  };
};
```
...
- Introduce `secretclaw/plugin-sdk`.
- Add `api.runtime` to `SecretClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `secretclawRuntime: ">=2026.2.0"`).
