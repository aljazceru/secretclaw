---
summary: "SecretClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SecretClaw on a Raspberry Pi
  - Running SecretClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# SecretClaw on Raspberry Pi

## Goal

Run a persistent, always-on SecretClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most SecretClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled secretclaw

# Enable if not
sudo systemctl enable secretclaw

# Start on boot
sudo systemctl start secretclaw
```
...
```bash
# Check logs
journalctl -u secretclaw --no-pager -n 100

# Common fix: rebuild
cd ~/secretclaw  # if using hackable install
npm run build
sudo systemctl restart secretclaw
```
