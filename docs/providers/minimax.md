---
summary: "Use MiniMax M2.1 in SecretClaw"
read_when:
  - You want MiniMax models in SecretClaw
...
```bash
secretclaw plugins enable minimax-portal-auth  # skip if already loaded.
secretclaw gateway restart  # restart if gateway is already running
secretclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/secretclaw/secretclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `secretclaw.json`:
...
- Use `secretclaw models list` and `secretclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `secretclaw configure` and selecting **MiniMax M2.1**, or
