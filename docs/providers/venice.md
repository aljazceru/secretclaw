---
summary: "Use Venice AI privacy-focused models in SecretClaw"
read_when:
  - You want privacy-focused inference in SecretClaw
...
```bash
secretclaw onboard --auth-choice venice-api-key
```
...
```bash
secretclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, SecretClaw shows all available Venice models. Pick based on your needs:
...
```bash
secretclaw models set venice/claude-opus-45
secretclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
secretclaw models list | grep venice
```

## Configure via `secretclaw configure`

1. Run `secretclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SecretClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
secretclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
secretclaw chat --model venice/claude-opus-45

# Use uncensored model
secretclaw chat --model venice/venice-uncensored

# Use vision model with image
secretclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
secretclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `secretclaw models list` to see currently available models. Some models may be temporarily offline.
