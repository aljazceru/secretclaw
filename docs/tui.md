...
```bash
secretclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `secretclaw tui --deliver`
...
- Check the Gateway logs: `secretclaw logs --follow`.
- Confirm the agent can run: `secretclaw status` and `secretclaw models status`.
...
- No agents in picker: check `secretclaw agents list` and your routing config.
