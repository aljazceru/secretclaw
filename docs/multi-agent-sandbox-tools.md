...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `secretclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `secretclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=secretclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${SECRETCLAW_STATE_DIR:-$HOME/.secretclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
