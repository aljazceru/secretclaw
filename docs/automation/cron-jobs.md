...
- Jobs persist under `~/.secretclaw/cron/` so restarts don’t lose schedules.
...
secretclaw cron list
secretclaw cron run <job-id> --force
secretclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.secretclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `secretclaw cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, SecretClaw defaults to `announce`.
...
- Job store: `~/.secretclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.secretclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.secretclaw/cron/jobs.json",
...
secretclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
secretclaw cron edit <jobId> --agent ops
secretclaw cron edit <jobId> --clear-agent
```
...
```bash
secretclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`secretclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SECRETCLAW_SKIP_CRON`.
