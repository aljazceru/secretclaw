...
This guide summarizes a sane workflow for working on the pi integration in SecretClaw.
...
- Trigger the agent directly:
  - `pnpm secretclaw agent --message "Hello" --thinking low`
...
State lives under the SecretClaw state directory. Default is `~/.secretclaw`. If `SECRETCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `secretclaw.json` for config
