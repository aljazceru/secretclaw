...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SecretClaw
...
Running SecretClaw via Node with `tsx` fails at startup with:

```
[secretclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `secretclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch secretclaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node secretclaw.mjs status` works on Node 25.
