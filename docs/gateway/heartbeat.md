...
- During heartbeat runs, SecretClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `secretclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SecretClaw skips the heartbeat run entirely (no model call).
...
```bash
secretclaw system event --text "Check for urgent follow-ups" --mode now
```
