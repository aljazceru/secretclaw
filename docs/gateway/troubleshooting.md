---
summary: "Quick troubleshooting guide for common SecretClaw failures"
read_when:
...
When SecretClaw misbehaves, here's how to fix it.
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `secretclaw status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `secretclaw status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `secretclaw status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `secretclaw gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `secretclaw channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `secretclaw gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `secretclaw logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `secretclaw status --all` (it redacts tokens). If you paste `secretclaw status`, consider setting `SECRETCLAW_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  secretclaw models auth setup-token --provider anthropic
  ```
...
```bash
secretclaw models auth paste-token --provider anthropic
secretclaw models status
```
...
- Preferred: `secretclaw logs --follow`
- File logs (always): `/tmp/secretclaw/secretclaw-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$SECRETCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u secretclaw-gateway[-<profile>].service -n 200 --no-pager`
...
- Run the wizard and set the Gateway run mode to **Local**:
  ```bash
  secretclaw configure
  ```
- Or set it directly:
  ```bash
  secretclaw config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  secretclaw config set gateway.mode remote
  secretclaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `secretclaw setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.secretclaw/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `secretclaw doctor`
...
- then run `secretclaw sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `secretclaw gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `secretclaw gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `secretclaw gateway status` and `secretclaw doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `SECRETCLAW_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `secretclaw gateway status` shows a config mismatch**
...
- Fix: rerun `secretclaw gateway install --force` from the same `--profile` / `SECRETCLAW_STATE_DIR` you want the service to use.

**If `secretclaw gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `secretclaw doctor` to update it (or `secretclaw gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `SECRETCLAW_GATEWAY_TOKEN`) and restart the service.

**If `secretclaw gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/secretclaw` on disk.
...
Symptoms: `pwd` or file tools show `~/.secretclaw/sandboxes/...` even though you
...
SecretClaw intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `secretclaw models list` or
  `secretclaw models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${SECRETCLAW_CONFIG_PATH:-$HOME/.secretclaw/secretclaw.json}"
```
...
tail -f "$(ls -t /tmp/secretclaw/secretclaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
secretclaw pairing list <channel>
```
...
```bash
secretclaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.secretclaw/agents/<agentId>/sessions/
```
...
secretclaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
secretclaw gateway --verbose
```

If you’re logged out / unlinked:

```bash
secretclaw channels logout
trash "${SECRETCLAW_STATE_DIR:-$HOME/.secretclaw}/credentials" # if logout can't cleanly remove everything
secretclaw channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/secretclaw/secretclaw-*.log | head -1)" | tail -20
```

### High Memory Usage

SecretClaw keeps conversation history in memory.
...
SecretClaw now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `secretclaw doctor` reports every invalid entry.
- `secretclaw doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `secretclaw logs`, `secretclaw health`, `secretclaw status`, `secretclaw gateway status`, and `secretclaw gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/secretclaw/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `secretclaw models status` (CLI).
...
5. Run `secretclaw channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. SecretClaw scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/secretclaw/secretclaw/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.secretclaw.* still works)
```
...
Ensure the global `secretclaw` CLI is installed and matches the app version:

```bash
secretclaw --version
npm install -g secretclaw@<version>
```
...
#   ${SECRETCLAW_CONFIG_PATH:-$HOME/.secretclaw/secretclaw.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
secretclaw gateway --verbose
secretclaw channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                              |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/secretclaw/secretclaw-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                       |
| Gateway service logs (supervisor) | macOS: `$SECRETCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.secretclaw/logs/...`; profiles use `~/.secretclaw-<profile>/logs/...`)<br />Linux: `journalctl --user -u secretclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "SecretClaw Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$SECRETCLAW_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                                    |
| Media cache                       | `$SECRETCLAW_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                        |
| Credentials                       | `$SECRETCLAW_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                                  |
...
secretclaw gateway status --deep
...
tail -20 /tmp/secretclaw/secretclaw-*.log
```
...
trash "${SECRETCLAW_STATE_DIR:-$HOME/.secretclaw}"
secretclaw channels login         # re-pair WhatsApp
secretclaw gateway restart           # or: secretclaw gateway
```
...
1. Check logs first: `/tmp/secretclaw/` (default: `secretclaw-YYYY-MM-DD.log`, or your configured `logging.file`)
