...
- Replaces the legacy `gateway` command. CLI entry point: `secretclaw gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
secretclaw gateway --port 18789
# for full debug/trace logs in stdio:
secretclaw gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
secretclaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.secretclaw/secretclaw.json` (or `SECRETCLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__secretclaw__/canvas/` from `~/.secretclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `SECRETCLAW_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `SECRETCLAW_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `SECRETCLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.secretclaw.*` may still exist)
- Linux: `secretclaw-gateway-<profile>.service`
- Windows: `SecretClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `SECRETCLAW_SERVICE_MARKER=secretclaw`
- `SECRETCLAW_SERVICE_KIND=gateway`
...
```bash
secretclaw --dev setup
secretclaw --dev gateway --allow-unconfigured
# then target the dev instance:
secretclaw --dev status
secretclaw --dev health
```

Defaults (can be overridden via env/flags/config):

- `SECRETCLAW_STATE_DIR=~/.secretclaw-dev`
- `SECRETCLAW_CONFIG_PATH=~/.secretclaw-dev/secretclaw.json`
- `SECRETCLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.secretclaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `SECRETCLAW_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `SECRETCLAW_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `SECRETCLAW_CONFIG_PATH`
- unique `SECRETCLAW_STATE_DIR`
...
```bash
secretclaw --profile main gateway install
secretclaw --profile rescue gateway install
```

Example:

```bash
SECRETCLAW_CONFIG_PATH=~/.secretclaw/a.json SECRETCLAW_STATE_DIR=~/.secretclaw-a secretclaw gateway --port 19001
SECRETCLAW_CONFIG_PATH=~/.secretclaw/b.json SECRETCLAW_STATE_DIR=~/.secretclaw-b secretclaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `secretclaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `secretclaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `secretclaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.secretclaw.*` is cleaned up).
  - `secretclaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are SecretClaw profile services.
  ...
  - Cleanup: `secretclaw gateway uninstall` (current service) and `secretclaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `secretclaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- SecretClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.secretclaw.*` labels still unload cleanly).
- To stop it cleanly, use `secretclaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `secretclaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `secretclaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

SecretClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`secretclaw gateway install` writes the user unit. `secretclaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/secretclaw-gateway[-<profile>].service`:

```
[Unit]
Description=SecretClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/secretclaw gateway --port 18789
Restart=always
RestartSec=5
Environment=SECRETCLAW_GATEWAY_TOKEN=
...
```
systemctl --user enable --now secretclaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/secretclaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now secretclaw-gateway[-<profile>].service
```
...
- `secretclaw gateway health|status` — request health/status over the Gateway WS.
- `secretclaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `secretclaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `secretclaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `secretclaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `secretclaw gateway` and the legacy TCP control port.
