---
summary: "All configuration options for ~/.secretclaw/secretclaw.json with examples"
read_when:
...
SecretClaw reads an optional **JSON5** config from `~/.secretclaw/secretclaw.json` (comments + trailing commas allowed).

If the file is missing, SecretClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.secretclaw/workspace`). You usually only need a config to:
...
SecretClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `secretclaw doctor`, `secretclaw logs`, `secretclaw health`, `secretclaw status`, `secretclaw service`, `secretclaw help`).
- Run `secretclaw doctor` to see the exact issues.
- Run `secretclaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `secretclaw config set`. Keep a backup of `~/.secretclaw/secretclaw.json`.
...
```bash
secretclaw gateway call config.get --params '{}' # capture payload.hash
secretclaw gateway call config.apply --params '{
...
```bash
secretclaw gateway call config.get --params '{}' # capture payload.hash
secretclaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.secretclaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@secretclaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.secretclaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.secretclaw/workspace" }],
}
```
...
{ "$include": "/etc/secretclaw/base.json5" } // absolute
...
```json5
// ~/.secretclaw/clients/mueller/agents.json5
[
...
```json5
// ~/.secretclaw/clients/mueller/broadcast.json5
{
...
SecretClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, SecretClaw runs your login shell and imports only the missing expected keys (never overrides).
...
- `SECRETCLAW_LOAD_SHELL_ENV=1`
- `SECRETCLAW_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${SECRETCLAW_GATEWAY_TOKEN}",
    },
  },
}
```
...
SecretClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.secretclaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.secretclaw/credentials/oauth.json` (or `$SECRETCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.secretclaw/agent/*` (migrated by `secretclaw doctor` into `~/.secretclaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `SECRETCLAW_OAUTH_DIR`
- Agent dir (default agent root override): `SECRETCLAW_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, SecretClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, SecretClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/secretclaw/secretclaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/secretclaw/secretclaw.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/secretclaw/secretclaw.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.secretclaw/credentials/whatsapp/biz
          // authDir: "~/.secretclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `secretclaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@secretclaw", "secretclaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@secretclaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.secretclaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.secretclaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.secretclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.secretclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.secretclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.secretclaw/workspace-home" },
      { id: "work", workspace: "~/.secretclaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `secretclaw.json`).
...
SecretClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["secretclaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-secretclaw",
...
SecretClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `secretclaw plugins install @secretclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
SecretClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
SecretClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, SecretClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/secretclaw" } },
}
```
...
When a file exceeds this limit, SecretClaw logs a warning and injects a truncated
...
message envelopes). If unset, SecretClaw uses the host timezone at runtime.
...
`[secretclaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[secretclaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[secretclaw]"`, SecretClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, SecretClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.secretclaw/settings/tts.json",
      elevenlabs: {
...
SecretClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, SecretClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `secretclaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:secretclaw`: all built-in SecretClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.secretclaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.secretclaw/sandboxes",
        docker: {
          image: "secretclaw-sandbox:bookworm-slim",
          containerPrefix: "secretclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "secretclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "secretclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "secretclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
SecretClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.secretclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
SecretClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, SecretClaw writes/merges a `models.json` into
`~/.secretclaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. SecretClaw uses
...
- Shortcut: `secretclaw onboard --auth-choice opencode-zen`.
...
Shortcut: `secretclaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `secretclaw onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `secretclaw onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `secretclaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `secretclaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `secretclaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `SECRETCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.secretclaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.secretclaw/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.secretclaw/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, SecretClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.secretclaw/skills` (workspace skills
...
from `~/.secretclaw/extensions`, `<workspace>/.secretclaw/extensions`, plus any
...
### `browser` (secretclaw-managed browser)

SecretClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for secretclaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (SecretClaw.app menubar, or `secretclaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      secretclaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/secretclaw" }
...
- Examples: `"/ui"`, `"/secretclaw"`, `"/apps/secretclaw"`.
...
- When a connection comes from one of these IPs, SecretClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `secretclaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `SECRETCLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `SECRETCLAW_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. SecretClaw
...
- SecretClaw.app watches `~/.secretclaw/secretclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.secretclaw/secretclaw.json` (or `SECRETCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.secretclaw/secretclaw.json` (or `SECRETCLAW_CONFIG_PATH`)
...
- `SECRETCLAW_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `secretclaw --dev …` → uses `~/.secretclaw-dev` + shifts ports from base `19001`
- `secretclaw --profile <name> …` → uses `~/.secretclaw-<name>` (port via config/env/flags)
...
```bash
SECRETCLAW_CONFIG_PATH=~/.secretclaw/a.json \
SECRETCLAW_STATE_DIR=~/.secretclaw-a \
secretclaw gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.secretclaw/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Gmail helper config (used by `secretclaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "secretclaw@gmail.com",
...
- Set `SECRETCLAW_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, SecretClaw defaults `serve.path` to `/` so
...
Default root: `~/.secretclaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the secretclaw browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__secretclaw__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__secretclaw__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.secretclaw/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `SECRETCLAW_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.secretclaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.secretclaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.secretclaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_secretclaw-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `secretclaw` (advertises `secretclaw.local`). Override with `SECRETCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_secretclaw-gw._tcp` under `~/.secretclaw/dns/` using the configured discovery domain (example: `secretclaw.internal.`).
