...
read_when:
  - "You installed SecretClaw but `secretclaw` is “command not found”"
...
SecretClaw’s runtime baseline is **Node 22+**.

If you can run `npm install -g secretclaw@latest` but later see `secretclaw: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
