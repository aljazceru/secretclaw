...
SecretClaw ships two installer scripts (served from `openclaw.ai`):
...
If the installer completes but `secretclaw` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- If upgrading an existing install: runs `secretclaw doctor --non-interactive` (best effort).
- For git installs: runs `secretclaw doctor --non-interactive` after install/update (best effort).
...
If you run the installer while **already inside a SecretClaw source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:

- update and use this checkout (`git`)
- or migrate to the global npm install (`npm`)

In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `SECRETCLAW_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `secretclaw` into a prefix (default: `~/.secretclaw`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g secretclaw@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `secretclaw doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://openclaw.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\secretclaw"
```
...
- **"secretclaw" is not recognized**: your npm global bin folder is not on PATH. Most systems use
