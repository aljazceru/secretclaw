...
read_when:
  - You want to understand what “context” means in SecretClaw
...
“Context” is **everything SecretClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (SecretClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How SecretClaw builds the system prompt

The system prompt is **SecretClaw-owned** and rebuilt each run. It includes:
...
By default, SecretClaw injects a fixed set of workspace files (if present):
