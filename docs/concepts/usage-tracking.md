...
- `/usage cost` in chats: local cost summary aggregated from SecretClaw session logs.
- CLI: `secretclaw status --usage` prints a full per-provider breakdown.
- CLI: `secretclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
