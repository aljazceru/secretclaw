...
Deploy SecretClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/secretclaw/secretclaw"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run SecretClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SECRETCLAW_STATE_DIR
        value: /data/.secretclaw
      - key: SECRETCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: SECRETCLAW_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: secretclaw-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/secretclaw`.
...
If you use the original SecretClaw repository, Render will not auto-deploy your SecretClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For SecretClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SecretClaw host.
