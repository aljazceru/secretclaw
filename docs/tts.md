...
SecretClaw can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere SecretClaw can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, SecretClaw defaults
...
TTS config lives under `messages.tts` in `secretclaw.json`.
...
- If `provider` is **unset**, SecretClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.secretclaw/settings/tts.json`, override with `SECRETCLAW_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, SecretClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, SecretClaw:
...
Discord note: `/tts` is a built-in Discord command, so SecretClaw registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from SecretClaw
```
