...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/secretclaw`)
...
If the page fails to load, start the Gateway first: `secretclaw gateway`.
...
you revoke it with `secretclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.secretclaw/secretclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SecretClaw
...
SecretClaw **blocks** Control UI connections without device identity.
