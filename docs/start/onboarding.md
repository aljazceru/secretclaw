---
summary: "First-run onboarding flow for SecretClaw (macOS app)"
read_when:
...
- Writes credentials to `~/.secretclaw/credentials/oauth.json`
...
The app can install the global `secretclaw` CLI via npm/pnpm so terminal
...
On the first agent run, SecretClaw bootstraps a workspace (default `~/.secretclaw/workspace`):
...
```bash
secretclaw webhooks gmail setup --account you@gmail.com
```
...
- `~/.secretclaw/credentials/oauth.json`
