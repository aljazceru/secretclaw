...
struct IOSGatewayChatTransport: SecretClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SecretClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SecretClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SecretClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SecretClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SecretClawChatAttachmentPayload]) async throws -> SecretClawChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SecretClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(SecretClawChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(SecretClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SecretClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SecretClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SecretClawAgentEventPayload.self)
                        {
