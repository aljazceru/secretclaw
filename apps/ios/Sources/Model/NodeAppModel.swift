...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SecretClawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SecretClawCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SecretClawCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SecretClawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SecretClawCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__secretclaw__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: SecretClawLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for secretclaw:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case SecretClawLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case SecretClawCanvasCommand.present.rawValue,
            ...
            case SecretClawCanvasA2UICommand.reset.rawValue,
            ...
            case SecretClawCameraCommand.list.rawValue,
                 SecretClawCameraCommand.snap.rawValue,
                 SecretClawCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case SecretClawScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SecretClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SecretClawNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SecretClawLocationGetParams.self, from: req.paramsJSON)) ??
            SecretClawLocationGetParams()
        ...
        let payload = SecretClawLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SecretClawCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(SecretClawCanvasPresentParams.self, from: req.paramsJSON)) ??
                SecretClawCanvasPresentParams()
        ...
        case SecretClawCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SecretClawCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SecretClawCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SecretClawCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SecretClawCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SecretClawCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SecretClawCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SecretClawCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing secretclawA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SecretClawCanvasA2UICommand.push.rawValue, SecretClawCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == SecretClawCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(SecretClawCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try SecretClawCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(SecretClawCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SecretClawCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.secretclawA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SecretClawCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SecretClawCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SecretClawCameraSnapParams.self, from: req.paramsJSON)) ??
                SecretClawCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SecretClawCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SecretClawCameraClipParams.self, from: req.paramsJSON)) ??
                SecretClawCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SecretClawScreenRecordParams.self, from: req.paramsJSON)) ??
            SecretClawScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> SecretClawLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SecretClawLocationMode(rawValue: raw) ?? .off
    }
