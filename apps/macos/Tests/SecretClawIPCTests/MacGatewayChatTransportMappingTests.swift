...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: SecretClawProtocol.AnyCodable(["ok": SecretClawProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SecretClawProtocol.AnyCodable(["ok": SecretClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = SecretClawProtocol.AnyCodable([
            "runId": SecretClawProtocol.AnyCodable("run-1"),
            "sessionKey": SecretClawProtocol.AnyCodable("main"),
            "state": SecretClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SecretClawProtocol.AnyCodable(["a": SecretClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
