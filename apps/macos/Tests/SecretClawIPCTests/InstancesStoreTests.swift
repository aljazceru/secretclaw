...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SecretClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SecretClawProtocol.AnyCodable] = [
            "presence": .init([SecretClawProtocol.AnyCodable(entry)]),
        ]
        let payload = SecretClawProtocol.AnyCodable(payloadMap)
