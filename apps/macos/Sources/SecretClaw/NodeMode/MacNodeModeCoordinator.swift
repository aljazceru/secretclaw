...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.secretclaw", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SecretClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [SecretClawCapability.canvas.rawValue, SecretClawCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(SecretClawCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if SecretClawLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(SecretClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(SecretClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(SecretClawCapability.location.rawValue) {
