...
private let webChatSwiftLogger = Logger(subsystem: "ai.secretclaw", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SecretClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SecretClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SecretClawChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SecretClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SecretClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SecretClawChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SecretClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SecretClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SecretClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SecretClawChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SecretClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SecretClawChatView>) -> NSViewController
    {
