...
enum CommandResolver {
    private static let projectRootDefaultsKey = "secretclaw.gatewayProjectRootPath"
    private static let helperName = "secretclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let secretclawEntry = root.appendingPathComponent("secretclaw.mjs").path
        if FileManager().isReadableFile(atPath: secretclawEntry) { return secretclawEntry }
        let binEntry = root.appendingPathComponent("bin/secretclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/secretclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let secretclawPaths = self.secretclawManagedPaths(home: home)
        if !secretclawPaths.isEmpty {
            extras.insert(contentsOf: secretclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + secretclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func secretclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".secretclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func secretclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSecretClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("secretclaw.mjs").path,
            root.appendingPathComponent("bin/secretclaw.js").path,
        ]
    ...
    static func hasAnySecretClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.secretclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func secretclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let secretclawPath = self.projectSecretClawExecutable(projectRoot: root) {
                return [secretclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "secretclaw", subcommand] + extraArgs
            }
            if let secretclawPath = self.secretclawExecutable(searchPaths: searchPaths) {
                return [secretclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            secretclaw entrypoint missing (looked for dist/index.js or secretclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real secretclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/secretclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v secretclaw >/dev/null 2>&1; then
          CLI="$(command -v secretclaw)"
          secretclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/secretclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/secretclaw.mjs"
            node "$PRJ/secretclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/secretclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/secretclaw.js"
            node "$PRJ/bin/secretclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent secretclaw"
          pnpm --silent secretclaw \(quotedArgs);
        else
          echo "secretclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SecretClawConfigFile.loadDict()
