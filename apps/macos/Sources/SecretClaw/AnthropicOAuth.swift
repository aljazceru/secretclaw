...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (SecretClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SecretClawOAuthStore.AnthropicOAuthStatus = SecretClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.secretclaw", category: "anthropic-oauth")
...
enum SecretClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let secretclawOAuthDirEnv = "SECRETCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "SecretClaw OAuth token file not found"
            case .unreadableFile: "SecretClaw OAuth token file not readable"
            case .invalidJSON: "SecretClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in SecretClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "SecretClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.secretclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".secretclaw", isDirectory: true)
