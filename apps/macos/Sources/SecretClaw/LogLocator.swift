...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SECRETCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/secretclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("secretclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("secretclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/secretclaw/ (rolling or stdout), or nil if none exist.
