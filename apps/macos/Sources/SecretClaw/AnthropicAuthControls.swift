...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SecretClawOAuthStore.AnthropicOAuthStatus = SecretClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SecretClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SecretClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SecretClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SecretClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. SecretClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SecretClawOAuthStore.AnthropicOAuthStatus,
