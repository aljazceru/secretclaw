...
enum DebugActions {
    private static let verboseDefaultsKey = "secretclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "secretclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SecretClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SecretClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SecretClawPaths.configURL
        guard
