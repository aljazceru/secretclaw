...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.secretclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SecretClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SecretClawSessionPreviewEntry,
    ...
    private static func cache(payload: SecretClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SecretClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SecretClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SecretClawKit.AnyCodable]) -> [SecretClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SecretClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SecretClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SecretClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SecretClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SecretClawChatMessage) -> String? {
