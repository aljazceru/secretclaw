...
enum SecretClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SecretClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SecretClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SecretClawLogging.bootstrapIfNeeded()
        let label = SecretClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SecretClawOSLogHandler: LogHandler {
...
struct SecretClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SecretClawLogging.parseLabel(self.label)
