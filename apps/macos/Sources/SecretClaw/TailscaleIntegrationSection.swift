...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.secretclaw/secretclaw.json. Prefer SECRETCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.secretclaw/secretclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.secretclaw/secretclaw.json. Restart the gateway to apply."
        }
