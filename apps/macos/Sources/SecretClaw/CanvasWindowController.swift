...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__secretclawA2UIBridgeInstalled) return;
            globalThis.__secretclawA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.secretclawCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  !!globalThis.secretclawA2UI ||
                  !!document.querySelector('secretclaw-a2ui-host');
                ...
                location.href = 'secretclaw://agent?' + params.toString();
              } catch {}
            }, true);
          } catch {}
        })();
    ...
    func applyDebugStatusIfNeeded() {
        ...
        (() => {
          try {
            const api = globalThis.__secretclaw;
            if (!api) return;
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/secretclaw-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
