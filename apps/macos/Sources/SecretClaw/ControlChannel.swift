...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SecretClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.secretclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SecretClawKit.AnyCodable]()) {
                $0[$1.key] = SecretClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SecretClawProtocol.AnyCodable?) -> [String: SecretClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SecretClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SecretClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SecretClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("secretclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("secretclaw.control.agent")
}
