...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.secretclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SecretClawPeekabooBridgeServices?
    private static var secretclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("SecretClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SecretClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.secretclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.secretclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SecretClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.secretclaw.peekaboo")
