...
import Foundation
import Testing
@testable import SecretClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, SecretClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SecretClawChatHistoryPayload]
    private let sessionsResponses: [SecretClawChatSessionsListResponse]

    private let stream: AsyncStream<SecretClawChatTransportEvent>
    private let continuation: AsyncStream<SecretClawChatTransportEvent>.Continuation

    init(
        historyResponses: [SecretClawChatHistoryPayload],
        sessionsResponses: [SecretClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SecretClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SecretClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SecretClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SecretClawChatAttachmentPayload]) async throws -> SecretClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SecretClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SecretClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SecretClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SecretClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SecretClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SecretClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SecretClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SecretClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SecretClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SecretClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SecretClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
