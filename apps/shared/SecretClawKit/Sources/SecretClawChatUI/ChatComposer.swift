...
@MainActor
struct SecretClawChatComposer: View {
    @Bindable var viewModel: SecretClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SecretClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SecretClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SecretClawPendingAttachment.id)
                { (att: SecretClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SecretClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(SecretClawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SecretClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SecretClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SecretClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
