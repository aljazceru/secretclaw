...
struct ChatMessageBubble: View {
    let message: SecretClawChatMessage
    let style: SecretClawChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SecretClawChatTheme.userText : SecretClawChatTheme.assistantText
    ...
    private var inlineAttachments: [SecretClawChatMessageContent] {
    ...
    private var toolCalls: [SecretClawChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SecretClawChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SecretClawChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SecretClawChatTheme.onboardingAssistantBubble
        }
        return SecretClawChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SecretClawChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SecretClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SecretClawChatTheme.userText : SecretClawChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SecretClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SecretClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SecretClawChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SecretClawChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SecretClawChatTheme.assistantText)
            }
        }
    }
}
