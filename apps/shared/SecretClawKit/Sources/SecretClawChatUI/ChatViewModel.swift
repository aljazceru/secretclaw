...
private let chatUILogger = Logger(subsystem: "ai.secretclaw", category: "SecretClawChatUI")

@MainActor
@Observable
public final class SecretClawChatViewModel {
    public private(set) var messages: [SecretClawChatMessage] = []
    ...
    public var attachments: [SecretClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SecretClawChatPendingToolCall] = []
    public private(set) var sessions: [SecretClawChatSessionEntry] = []
    private let transport: any SecretClawChatTransport
    ...
    private var pendingToolCallsById: [String: SecretClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SecretClawChatTransport) {
    ...
    public var sessionChoices: [SecretClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SecretClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SecretClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SecretClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SecretClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SecretClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SecretClawChatMessage]) -> [SecretClawChatMessage] {
        var result: [SecretClawChatMessage] = []
    ...
    private static func dedupeKey(for message: SecretClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SecretClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SecretClawChatAttachmentPayload in
            SecretClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SecretClawChatSessionEntry {
        SecretClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SecretClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SecretClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SecretClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SecretClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SecretClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
