# 🦞 SecretClaw — Personal AI Assistant

<p align="center">
    <picture>
        <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/secretclaw/secretclaw/main/docs/assets/secretclaw-logo-text-dark.png">
        <img src="https://raw.githubusercontent.com/secretclaw/secretclaw/main/docs/assets/secretclaw-logo-text.png" alt="SecretClaw" width="500">
    </picture>
</p>
...
<p align="center">
  <a href="https://github.com/secretclaw/secretclaw/actions/workflows/ci.yml?branch=main"><img src="https://img.shields.io/github/actions/workflow/status/secretclaw/secretclaw/ci.yml?branch=main&style=for-the-badge" alt="CI status"></a>
  <a href="https://github.com/secretclaw/secretclaw/releases"><img src="https://img.shields.io/github/v/release/secretclaw/secretclaw?include_prereleases&style=for-the-badge" alt="GitHub release"></a>
  <a href="https://discord.gg/clawd"><img src="https://img.shields.io/discord/1456350064065904867?label=Discord&logo=discord&logoColor=white&color=5865F2&style=for-the-badge" alt="Discord"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=for-the-badge" alt="MIT License"></a>
</p>

**SecretClaw** is a _personal AI assistant_ you run on your own devices.
It answers you on the channels you already use (WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat), plus extension channels like BlueBubbles, Matrix, Zalo, and Zalo Personal. It can speak and listen on macOS/iOS/Android, and can render a live Canvas you control. The Gateway is just the control plane — the product is the assistant.

If you want a personal, single-user assistant that feels local, fast, and always-on, this is it.

[Website](https://openclaw.ai) · [Docs](https://docs.openclaw.ai) · [DeepWiki](https://deepwiki.com/secretclaw/secretclaw) · [Getting Started](https://docs.openclaw.ai/start/getting-started) · [Updating](https://docs.openclaw.ai/install/updating) · [Showcase](https://docs.openclaw.ai/start/showcase) · [FAQ](https://docs.openclaw.ai/start/faq) · [Wizard](https://docs.openclaw.ai/start/wizard) · [Nix](https://github.com/secretclaw/nix-clawdbot) · [Docker](https://docs.openclaw.ai/install/docker) · [Discord](https://discord.gg/clawd)

Preferred setup: run the onboarding wizard (`secretclaw onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
...
# or: pnpm add -g secretclaw@latest
...
# Send a message
secretclaw message send --to +1234567890 --message "Hello from SecretClaw"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
secretclaw agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.openclaw.ai/install/updating) (and run `secretclaw doctor`).
...
Switch channels (git + npm): `secretclaw update --channel stable|beta|dev`.
...
pnpm secretclaw onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm secretclaw ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `secretclaw` binary.

## Security defaults (DM access)

SecretClaw connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.
...
- Approve with: `secretclaw pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `secretclaw doctor` to surface risky/misconfigured DM policies.
...
[![Star History Chart](https://api.star-history.com/svg?repos=secretclaw/secretclaw&type=date&legend=top-left)](https://www.star-history.com/#secretclaw/secretclaw&type=date&legend=top-left)
...
- [Browser control](https://docs.openclaw.ai/tools/browser): dedicated secretclaw Chrome/Chromium, snapshots, actions, uploads, profiles.
...
- **[Browser control](https://docs.openclaw.ai/tools/browser)** — secretclaw‑managed Chrome/Chromium with CDP control.
...
SecretClaw can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (SecretClaw enforces this).
...
- Controlled via `secretclaw nodes …`.
...
- Workspace root: `~/.secretclaw/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.secretclaw/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.secretclaw/secretclaw.json` (model + defaults):
...
- Link the device: `pnpm secretclaw channels login` (stores creds in `~/.secretclaw/credentials`).
...
SecretClaw was built for **Molty**, a space lobster AI assistant. 🦞
by Peter Steinberger and the community.

- [openclaw.ai](https://openclaw.ai)
- [soul.md](https://soul.md)
- [steipete.me](https://steipete.me)
- [@secretclaw](https://x.com/secretclaw)
