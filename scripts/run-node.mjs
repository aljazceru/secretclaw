...
const shouldBuild = () => {
  if (env.SECRETCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.SECRETCLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[secretclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["secretclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
