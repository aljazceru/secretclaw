...
#
# Suggested cron: */30 * * * * /home/admin/secretclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SecretClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., secretclaw-alerts)
...
STATE_FILE="$HOME/.secretclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SecretClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use secretclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SecretClaw to $NOTIFY_PHONE..."
            secretclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SecretClaw is down. Run: ssh l36 '~/secretclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
