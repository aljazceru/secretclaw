#!/bin/bash
# Setup SecretClaw Auth Management System
...
echo "=== SecretClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'secretclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SecretClaw message: Send warning via SecretClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/secretclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/secretclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/secretclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now secretclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status secretclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
