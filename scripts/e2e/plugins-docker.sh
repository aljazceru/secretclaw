...
IMAGE_NAME="secretclaw-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/secretclaw-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.secretclaw/extensions/demo-plugin"

  cat > "$HOME/.secretclaw/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.secretclaw/extensions/demo-plugin/secretclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/secretclaw-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@secretclaw/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/secretclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/secretclaw-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/secretclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/secretclaw-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@secretclaw/demo-plugin-file",
  "version": "0.0.1",
  "secretclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/secretclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
