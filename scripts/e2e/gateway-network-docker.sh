...
IMAGE_NAME="secretclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="secretclaw-net-e2e-$$"
GW_NAME="secretclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SECRETCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "SECRETCLAW_SKIP_CHANNELS=1" \
	  -e "SECRETCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "SECRETCLAW_SKIP_CRON=1" \
	  -e "SECRETCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
