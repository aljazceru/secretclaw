...
IMAGE_NAME="secretclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/secretclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/secretclaw-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/secretclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/secretclaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/secretclaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.secretclaw/workspace"
    config_path="$HOME/.secretclaw/secretclaw.json"
    sessions_dir="$HOME/.secretclaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.secretclaw/secretclaw.json" <<'"'"'JSON'"'"'
{
