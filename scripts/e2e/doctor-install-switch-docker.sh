...
IMAGE_NAME="secretclaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/secretclaw-bin:$PATH"
  mkdir -p /tmp/secretclaw-bin

  cat > /tmp/secretclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/secretclaw-bin/systemctl

  cat > /tmp/secretclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/secretclaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/secretclaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/secretclaw/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/secretclaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/secretclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/secretclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
