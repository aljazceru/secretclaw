...
# Build and bundle SecretClaw into a minimal .app we can open.
# Outputs to dist/SecretClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SecretClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SecretClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.secretclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/secretclaw/secretclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SecretClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SecretClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SecretClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SecretClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SecretClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SecretClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SecretClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SecretClaw/Resources/SecretClaw.icns" "$APP_ROOT/Contents/Resources/SecretClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SecretClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SecretClawKit resources"
SECRETCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SecretClawKit_SecretClawKit.bundle"
if [ -d "$SECRETCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SecretClawKit_SecretClawKit.bundle"
  cp -R "$SECRETCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SecretClawKit_SecretClawKit.bundle"
else
  echo "WARN: SecretClawKit resource bundle not found at $SECRETCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SecretClaw"
killall -q SecretClaw 2>/dev/null || true
