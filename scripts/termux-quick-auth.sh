...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/secretclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SecretClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/secretclaw/scripts/mobile-reauth.sh'" --id secretclaw-auth
        ;;
    *)
