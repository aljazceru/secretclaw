...
INSTALL_URL="${SECRETCLAW_INSTALL_URL:-https://openclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SECRETCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SECRETCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="secretclaw"
PACKAGE_NAME="${SECRETCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SECRETCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SECRETCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SECRETCLAW_INSTALL_LATEST_OUT:-}"
fi
