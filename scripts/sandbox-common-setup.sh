...
BASE_IMAGE="${BASE_IMAGE:-secretclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-secretclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=secretclaw.sandbox=1)
NOTE
