# SecretClaw ACP Bridge

This document describes how the SecretClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`secretclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
SecretClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a SecretClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `secretclaw acp` over stdio.

Example config:

```bash
secretclaw config set gateway.remote.url wss://gateway-host:18789
secretclaw config set gateway.remote.token <token>
```

Example run:

```bash
secretclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
secretclaw acp --session agent:main:main
secretclaw acp --session agent:design:main
secretclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “SecretClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `secretclaw acp` and speaks ACP messages over stdio.
...
```bash
secretclaw acp --session agent:main:main
secretclaw acp --session-label "support inbox"
secretclaw acp --reset-session
```
...
`secretclaw acp` resolves the Gateway URL and auth from CLI flags or config:
