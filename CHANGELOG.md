...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (secretclaw-cn).
...
- Docs: finish renaming the QMD memory docs to reference the SecretClaw state dir.
...
- Skills: update session-logs paths from .clawdbot to .secretclaw. (#4502)
...
- Build: align npm tar override and bin metadata so the `secretclaw` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update SecretClaw branding + install links.
...
- Rebrand: rename the npm package/CLI to `secretclaw`, add a `secretclaw` compatibility shim, and move extensions to the `@secretclaw/*` scope.
...
- macOS: finish SecretClaw app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/secretclaw/secretclaw/pull/3047) Thanks @zackerthescar.
...
- Skills: update session-logs paths to use ~/.secretclaw. (#4502) Thanks @bonald.
...
- CLI: add `secretclaw system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.openclaw.ai/cli/system
- CLI: add live auth probes to `secretclaw models status` for per-profile verification. (commit 40181afde) https://docs.openclaw.ai/cli/models
- CLI: restart the gateway by default after `secretclaw update`; add `--no-restart` to skip it. (commit 2c85b1b40)
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `secretclaw models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- CLI: flatten node service commands under `secretclaw node` and remove `service node` docs.
- CLI: move gateway service commands under `secretclaw gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `secretclaw update wizard` for interactive channel selection and restart prompts. https://docs.openclaw.ai/cli/update
...
- ACP: add `secretclaw acp` for IDE integrations. https://docs.openclaw.ai/cli/acp
- ACP: add `secretclaw acp client` interactive harness for debugging. https://docs.openclaw.ai/cli/acp
...
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `secretclaw update`.
- Plugins: share npm plugin update logic between `secretclaw update` and `secretclaw plugins update`.
...
- Exec approvals: migrate approvals to `~/.secretclaw/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.openclaw.ai/tools/exec-approvals
- Nodes: add headless node host (`secretclaw node start`) for `system.run`/`system.which`. https://docs.openclaw.ai/cli/node
...
- Build: use tsgo for dev/watch builds by default (opt out with `SECRETCLAW_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `secretclaw doctor --fix` to repair, then update plugins (`secretclaw plugins update`) if you use any.
...
- CLI: keep `secretclaw logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `secretclaw memory status`.
...
- Memory: probe sqlite-vec availability in `secretclaw memory status`.
...
- Plugins: add Zalo Personal plugin (`@secretclaw/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.openclaw.ai/plugins/zalouser
...
- **BREAKING:** `secretclaw message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `secretclaw hooks` is now `secretclaw webhooks`; hooks live under `secretclaw hooks`. https://docs.openclaw.ai/cli/webhooks
- **BREAKING:** `secretclaw plugins install <path>` now copies into `~/.secretclaw/extensions` (use `--link` to keep path-based loading).
...
- Directory: unify `secretclaw directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `secretclaw.hooks` manifests and `secretclaw hooks install/update`.
...
- CLI: speed up `secretclaw sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `secretclaw models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@secretclaw/msteams` via `secretclaw plugins install @secretclaw/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `secretclaw-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `secretclaw security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `secretclaw models status`, and update docs.
- CLI: add `--json` output for `secretclaw daemon` lifecycle/install commands.
...
- Fix: make `secretclaw update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `secretclaw configure`, even if no other sections are chosen.
...
- Security: expanded `secretclaw security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `secretclaw security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `secretclaw browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.secretclaw-dev`).
...
- Memory: new `secretclaw memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.secretclaw/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `secretclaw dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `secretclaw doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `secretclaw doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `secretclaw plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `secretclaw` CLI when missing in local mode; install via `openclaw.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `secretclaw.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `secretclaw status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `secretclaw update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `secretclaw reset` and `secretclaw uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `secretclaw gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `secretclaw status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `secretclaw sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `secretclaw sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “SecretClaw” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `secretclaw update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/secretclaw`. (#660)
...
- CLI: `secretclaw message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `secretclaw models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.secretclaw/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `secretclaw pairing list <provider>` + `secretclaw pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `secretclaw logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$SECRETCLAW_STATE_DIR/.env` (`~/.secretclaw/.env`) as a fallback after CWD `.env`.
...
- CLI: run `secretclaw agent` via the Gateway by default; use `--local` to force embedded mode.
